object FLocOC: TFLocOC
  Left = 192
  Top = 109
  AutoSize = True
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Localiza'#231#227'o de Ordens de Compra'
  ClientHeight = 393
  ClientWidth = 608
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 608
    Height = 393
    Align = alClient
    BevelInner = bvRaised
    BevelOuter = bvLowered
    TabOrder = 0
    object DBGrid1: TDBGrid
      Left = 2
      Top = 146
      Width = 601
      Height = 203
      DataSource = DSBuscaOrc
      TabOrder = 7
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'DATAORDEMCOMPRA'
          Title.Caption = 'Data OC.'
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Title.Caption = 'Nome'
          Width = 250
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRAZOENTREGA'
          Title.Caption = 'Prazo de Entrega'
          Width = 133
          Visible = True
        end>
    end
    object BitCancelar: TBitBtn
      Left = 516
      Top = 360
      Width = 80
      Height = 25
      Hint = 'Cancelar Busca'
      Caption = 'Cancelar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 9
      OnClick = BitCancelarClick
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
    end
    object BitBuscar: TBitBtn
      Left = 422
      Top = 360
      Width = 80
      Height = 25
      Hint = 'Buscar Ordem de Compra Selecionado'
      Caption = 'Buscar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = BitBuscarClick
      Glyph.Data = {
        
        077777777777777777777777777777777777}
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 137
      Top = 6
      Width = 263
      Height = 21
      KeyField = 'CODCADGEN'
      ListField = 'NOME'
      ListSource = Dados.DSFornecedoresLK
      TabOrder = 1
      OnEnter = DBLookupComboBox1Enter
      OnKeyDown = DBLookupComboBox1KeyDown
    end
    object CheckBox1: TCheckBox
      Left = 33
      Top = 8
      Width = 94
      Height = 17
      Caption = 'Fornecedor'
      TabOrder = 0
    end
    object CheckBox2: TCheckBox
      Left = 33
      Top = 30
      Width = 94
      Height = 17
      Caption = 'Contato'
      TabOrder = 2
    end
    object CheckBox3: TCheckBox
      Left = 33
      Top = 51
      Width = 101
      Height = 17
      Caption = 'Prazo de Entrega'
      TabOrder = 4
    end
    object GroupBox1: TGroupBox
      Left = 8
      Top = 88
      Width = 326
      Height = 40
      Caption = 'Data do Ordem de Compra'
      TabOrder = 6
      object CheckBox5: TCheckBox
        Left = 8
        Top = 16
        Width = 49
        Height = 17
        Caption = 'Inicial'
        TabOrder = 0
      end
      object DTPIni: TDateTimePicker
        Left = 56
        Top = 13
        Width = 87
        Height = 21
        CalAlignment = dtaLeft
        Date = 38606.580457419
        Time = 38606.580457419
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 1
      end
      object CheckBox6: TCheckBox
        Left = 161
        Top = 15
        Width = 43
        Height = 17
        Caption = 'Final'
        TabOrder = 2
      end
      object DTPFin: TDateTimePicker
        Left = 207
        Top = 13
        Width = 87
        Height = 21
        CalAlignment = dtaLeft
        Date = 38606.580457419
        Time = 38606.580457419
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 3
      end
    end
    object CBCont: TComboBox
      Left = 137
      Top = 29
      Width = 263
      Height = 21
      ItemHeight = 13
      TabOrder = 3
      OnKeyDown = DBLookupComboBox1KeyDown
    end
    object CBPrazoEnt: TComboBox
      Left = 137
      Top = 52
      Width = 263
      Height = 21
      ItemHeight = 13
      TabOrder = 5
      OnKeyDown = DBLookupComboBox1KeyDown
    end
    object BitBtn1: TBitBtn
      Left = 422
      Top = 104
      Width = 139
      Height = 25
      Hint = 'Iniciar Pesquisa pelo Filtro'
      Caption = 'Iniciar Pesquisa'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 10
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
    end
  end
  object DSBuscaOrc: TDataSource
    DataSet = QryBuscaOC
    Left = 112
    Top = 208
  end
  object QryBuscaOC: TIBQuery
    Database = Dados.Database
    Transaction = FOrdemCompra.TransactionOC
    SQL.Strings = (
      
        'select oc.codordemcompra, cg.nome, oc.dataordemcompra, oc.contat' +
        'o,'
      'oc.prazoentrega from ordemcompra OC'
      'inner join cadgen cg on cg.codcadgen = oc.codforngen'
      'where oc.codordemcompra is not null')
    Left = 120
    Top = 176
    object QryBuscaOCCODORDEMCOMPRA: TIntegerField
      FieldName = 'CODORDEMCOMPRA'
      Origin = '"ORDEMCOMPRA"."CODORDEMCOMPRA"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QryBuscaOCNOME: TIBStringField
      FieldName = 'NOME'
      Origin = '"CADGEN"."NOME"'
      Size = 75
    end
    object QryBuscaOCDATAORDEMCOMPRA: TDateField
      FieldName = 'DATAORDEMCOMPRA'
      Origin = '"ORDEMCOMPRA"."DATAORDEMCOMPRA"'
    end
    object QryBuscaOCCONTATO: TIBStringField
      FieldName = 'CONTATO'
      Origin = '"ORDEMCOMPRA"."CONTATO"'
      Size = 50
    end
    object QryBuscaOCPRAZOENTREGA: TIBStringField
      FieldName = 'PRAZOENTREGA'
      Origin = '"ORDEMCOMPRA"."PRAZOENTREGA"'
      Size = 25
    end
  end
  object QryPrazoEntr: TIBQuery
    Database = Dados.Database
    Transaction = FOrdemCompra.TransactionOC
    SQL.Strings = (
      
        'select distinct(O.prazoentrega) from ORCAMENTOS O where O.prazoe' +
        'ntrega is not null'
      'order by O.prazoentrega')
    Left = 312
    Top = 72
    object QryPrazoEntrPRAZOENTREGA: TIBStringField
      FieldName = 'PRAZOENTREGA'
      Origin = '"ORCAMENTOS"."PRAZOENTREGA"'
      Size = 25
    end
  end
  object QryContato: TIBQuery
    Database = Dados.Database
    Transaction = FOrdemCompra.TransactionOC
    SQL.Strings = (
      
        'select distinct(O.contato) from ORCAMENTOS O where O.Contato is ' +
        'not null'
      'order by O.Contato')
    Left = 240
    Top = 24
    object QryContatoCONTATO: TIBStringField
      FieldName = 'CONTATO'
      Origin = '"ORCAMENTOS"."CONTATO"'
      Size = 50
    end
  end
end

object FLocVendas: TFLocVendas
  Left = 192
  Top = 109
  AutoSize = True
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Localiza'#231#227'o de Vendas'
  ClientHeight = 393
  ClientWidth = 608
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 608
    Height = 393
    Align = alClient
    BevelInner = bvRaised
    BevelOuter = bvLowered
    TabOrder = 0
    object DBGrid1: TDBGrid
      Left = 2
      Top = 154
      Width = 601
      Height = 203
      DataSource = DSBuscaVen
      TabOrder = 10
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'DATAVENDA'
          Title.Caption = 'Data Venda'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOMECLI'
          Title.Caption = 'Nome'
          Width = 200
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOMEPGTO'
          Title.Caption = 'Forma Pgto.'
          Width = 200
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TIPONOTA'
          Title.Caption = 'Tipo Nota'
          Width = 52
          Visible = True
        end>
    end
    object BitCancelar: TBitBtn
      Left = 516
      Top = 362
      Width = 80
      Height = 25
      Hint = 'Cancelar Busca'
      Caption = 'Cancelar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 12
      OnClick = BitCancelarClick
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
    end
    object BitBuscar: TBitBtn
      Left = 422
      Top = 362
      Width = 80
      Height = 25
      Hint = 'Buscar Venda Selecionada'
      Caption = 'Buscar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 11
      OnClick = BitBuscarClick
      Glyph.Data = {
        
        077777777777777777777777777777777777}
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 129
      Top = 6
      Width = 262
      Height = 21
      KeyField = 'CODCADGEN'
      ListField = 'NOME'
      ListSource = Dados.DSClientesLK
      TabOrder = 1
      OnEnter = DBLookupComboBox1Enter
    end
    object CheckBox1: TCheckBox
      Left = 33
      Top = 8
      Width = 94
      Height = 17
      Caption = 'Cliente'
      TabOrder = 0
    end
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 129
      Top = 28
      Width = 262
      Height = 21
      KeyField = 'CODCFOP'
      ListField = 'NOME'
      ListSource = Dados.DSCFOPLK
      TabOrder = 4
      OnEnter = DBLookupComboBox2Enter
    end
    object CheckBox2: TCheckBox
      Left = 33
      Top = 30
      Width = 94
      Height = 17
      Caption = 'CFOP'
      TabOrder = 3
    end
    object DBLookupComboBox3: TDBLookupComboBox
      Left = 129
      Top = 50
      Width = 262
      Height = 21
      KeyField = 'CODCADGEN'
      ListField = 'NOME'
      ListSource = Dados.DSTransportadorasLK
      TabOrder = 6
      OnEnter = DBLookupComboBox3Enter
    end
    object CheckBox3: TCheckBox
      Left = 33
      Top = 52
      Width = 94
      Height = 17
      Caption = 'Transportadora'
      TabOrder = 5
    end
    object DBLookupComboBox4: TDBLookupComboBox
      Left = 129
      Top = 72
      Width = 262
      Height = 21
      KeyField = 'CODCONDPGTO'
      ListField = 'NOME'
      ListSource = Dados.DSCondPgtoLK
      TabOrder = 8
      OnEnter = DBLookupComboBox4Enter
    end
    object CheckBox4: TCheckBox
      Left = 33
      Top = 74
      Width = 94
      Height = 17
      Caption = 'Cond. de Pgto'
      TabOrder = 7
    end
    object GroupBox1: TGroupBox
      Left = 8
      Top = 96
      Width = 326
      Height = 40
      Caption = 'Data de Emiss'#227'o da NF'
      TabOrder = 9
      object CheckBox5: TCheckBox
        Left = 8
        Top = 16
        Width = 49
        Height = 17
        Caption = 'Inicial'
        TabOrder = 0
      end
      object DTPIni: TDateTimePicker
        Left = 56
        Top = 13
        Width = 87
        Height = 21
        CalAlignment = dtaLeft
        Date = 38606.580457419
        Time = 38606.580457419
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 1
      end
      object CheckBox6: TCheckBox
        Left = 161
        Top = 15
        Width = 43
        Height = 17
        Caption = 'Final'
        TabOrder = 2
      end
      object DTPFin: TDateTimePicker
        Left = 207
        Top = 13
        Width = 87
        Height = 21
        CalAlignment = dtaLeft
        Date = 38606.580457419
        Time = 38606.580457419
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 3
      end
    end
    object RadioGroup1: TRadioGroup
      Left = 392
      Top = 2
      Width = 212
      Height = 29
      Caption = 'Tipo de Nota'
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        'Todas'
        'Entrada'
        'Sa'#237'da')
      TabOrder = 2
    end
    object BitBtn1: TBitBtn
      Left = 422
      Top = 120
      Width = 139
      Height = 25
      Hint = 'Iniciar Pesquisa pelo Filtro'
      Caption = 'Iniciar Pesquisa'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 13
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
    end
  end
  object DSBuscaVen: TDataSource
    DataSet = QryBuscaVen
    Left = 112
    Top = 208
  end
  object QryBuscaVen: TIBQuery
    Database = Dados.Database
    Transaction = FVendas.TransactionVen
    SQL.Strings = (
      
        'select v.NroVenda, cg.nome as NomeCli, cp.nome as NomePgto, v.da' +
        'tavenda,'
      'v.tiponota from Vendas V'
      
        'inner join cadgen cg on cg.codcadgen = V.codcligen inner join co' +
        'ndpgto cp on cp.codcondpgto = v.codcondpgto where v.nrovenda is ' +
        'not null')
    Left = 112
    Top = 176
    object QryBuscaVenNOMECLI: TIBStringField
      FieldName = 'NOMECLI'
      Origin = '"CLIENTES"."NOME"'
      Size = 75
    end
    object QryBuscaVenNOMEPGTO: TIBStringField
      FieldName = 'NOMEPGTO'
      Origin = '"CONDPGTO"."NOME"'
      Size = 35
    end
    object QryBuscaVenDATAVENDA: TDateField
      FieldName = 'DATAVENDA'
      Origin = '"VENDAS"."DATAVENDA"'
    end
    object QryBuscaVenTIPONOTA: TIBStringField
      FieldName = 'TIPONOTA'
      Origin = '"VENDAS"."TIPONOTA"'
      Size = 1
    end
    object QryBuscaVenNROVENDA: TIntegerField
      FieldName = 'NROVENDA'
      Origin = '"VENDAS"."NROVENDA"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
  end
end
